...
if command -v apptainer > /dev/null 2>&1; then
    apptainer instance stop --all 2>/dev/null && echo "   ✅ Apptainer instances cleared" || true
elif command -v singularity > /dev/null 2>&1; then
    singularity instance stop --all 2>/dev/null && echo "   ✅ Singularity instances cleared" || true
fi
