...
start_nim_service() {
    ...
    local gpu_id=$3
    local image=$4
    local singularity_opts="${5:-}"   # extra --env flags for singularity instance start
    local startscript_cmd="${6:-}"    # command passed after instance name (exec "$@" workaround)
    ...
    # Start NIM instance using .def file's %startscript (or %runscript if no %startscript).
    # $startscript_cmd (if set) is passed after instance name as args to %startscript "$@".
...
# Triton NIM: NIM_HTTP_API_PORT sets the bind port (belt+suspenders with %environment)
...
# nim_llm_sdk NIM: pass start_server.sh --port as startscript_cmd (exec "$@" workaround).
# After image rebuild, %startscript handles this directly without startscript_cmd.
...
# nim_llm_sdk NIM: %startscript already has --port 1977 baked in.
